...
#pgwc
openssl genrsa -out $1/pgwc.key.pem 1024
openssl req -new -batch -out pgwc.csr.pem -key $1/pgwc.key.pem -subj /CN=pgwc.EPC_DOMAIN/C=KO/ST=Seoul/L=Nowon/O=Open5GS/OU=Tests
openssl ca -cert $1/cacert.pem -days 3650 -keyfile cakey.pem -in pgwc.csr.pem -out $1/pgwc.cert.pem -outdir . -batch
...
# ocs
openssl genrsa -out $1/ocs.key.pem 1024
openssl req -new -batch -out ocs.csr.pem -key $1/ocs.key.pem -subj /CN=ocs.EPC_DOMAIN/C=KO/ST=Seoul/L=Nowon/O=Open5GS/OU=Tests
openssl ca -cert $1/cacert.pem -days 3650 -keyfile cakey.pem -in ocs.csr.pem -out $1/ocs.cert.pem -outdir . -batch

rm -rf demoCA
rm -f 01.pem 02.pem 03.pem 04.pem
rm -f cakey.pem
rm -f mme.csr.pem hss.csr.pem pgwc.csr.pem pcrf.csr.pem ocs.csr.pem
